<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if($GetType == "Sectors"){
            ...
            if($GetSector != ""){
                $stmt = $conn->prepare('SELECT id, name, num_purchases FROM materials WHERE id_sector = :id_sector');
        ...
        if($Type == 1){
            ...
            $num = $data['num'] + 1;
            $stmt = $conn->prepare('UPDATE materials SET num_purchases = :num WHERE id = :id');
            ...
            echo $data;
            $date = date('Y-m-d');
            $stmt2 = $conn->prepare('INSERT INTO purchases (id_material, id_user, op, date) VALUES (:id, :id_user, 1, :date)');
            ...
            if( $data['num'] != 0){
                ...
                echo $data;
                $date = date('Y-m-d');
                $stmt2 = $conn->prepare('INSERT INTO purchases (id_material, id_user, op, date) VALUES (:id, :id_user, 0, :date)');
                $stmt2->bindParam(":id", $id);
                $stmt2->bindParam(":id_user", $id_user);
                $stmt2->bindParam(":date", $date);
                $stmt2->execute();
